...
"""
http_list = [
    'http://dm.takaratomy.co.jp/?product=dmrp01',
    'http://dm.takaratomy.co.jp/?product=dmrp02',
    'http://dm.takaratomy.co.jp/?product=dmrp03',
    'http://dm.takaratomy.co.jp/?product=dmrp04s',
    'http://dm.takaratomy.co.jp/?product=dmrp04m',
    'http://dm.takaratomy.co.jp/?product=dmrp05'
]
"""
http_list = ['http://dm.takaratomy.co.jp/product/dmex01/']
...
def main():
    ...
    for one_card in all_card_list:
        write_contents.append(one_card)

    write_txt(write_contents, "dmex01")
    # write_sqlite(write_contents, "dm_card")

    """
    one_card = get_one_card_detail("http://dm.takaratomy.co.jp/card/detail/?id=dmrp05-m02")
    # http://dm.takaratomy.co.jp/card/detail/?id=dmrp05-m01
    # http://dm.takaratomy.co.jp/card/detail/?id=dmrp05-063
...
def get_card_detail_list(url_list):
    ...
    for url in url_list:
        ...
        results = soup.find_all("div", class_="border clearfix")
        for result in results:
            data_href = root + result.find("a", onclick="return false;").get("data-href")
...
def get_one_card_detail(url):
    ...
    card_name = table.find("img").get("alt")
    dm_card.card_name = card_name

    race = table.find("td", class_="racetxt").get_text()
    dm_card.race = race

    power = table.find("td", class_="powertxt").get_text()
    dm_card.power = power

    civ = table.find("td", class_="civtxt").get_text()
    dm_card.civ = civ

    cost = table.find("td", class_="costtxt").get_text().replace(" マナ", "")
    dm_card.cost = cost

    abilitytxt = table.find_all("td", class_="abilitytxt")
    ability_list = []
    for li in abilitytxt:
        print(li)
        for txt in li:
            ability_list.append(format_text(txt.get_text()))
    dm_card.ability = ",".join(ability_list)

    # print(card_name, race, power, civ, cost, abilitytxt)
    print(card_name)
...
def write_sqlite(contents, file_name):
    con = sqlite3.connect('./DM_DATE.db')  # データベースに接続する
    cur = con.cursor()
